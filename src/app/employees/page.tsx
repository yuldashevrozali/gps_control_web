  ...
  const AllEmployees = () => {
    ...
    const clearDeviceId = async (agentId: number) => {
  try {
    const token = await getValidAccessToken();
    if (!token) {
    ...
    await axios.patch(
      `https://gps.mxsoft.uz/account/agents/${agentId}/clear-device_id/`,
      {}, // PATCH body bo‘sh, faqat endpoint bajaradi
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    toast.success("✅ Qurilma ID o‘chirildi");

    // ✅ Agar xohlasang, listni yangilab qo‘yish:
    setAgents((prev) =>
      prev.map((a) =>
        a.id === agentId ? { ...a, status: false } : a
      )
    );
  } catch (error) {
    console.error("❌ device_id tozalashda xatolik:", error);
    toast.error("❌ Qurilma ID ni o‘chirishda muammo yuz berdi");
  }
};
  ...
  <DropdownMenu>
    ...
    <DropdownMenuContent className="hello">
      <Button
        variant="ghost"
        className=" btn123 justify-start text-sm"
      ...
      <Button
        ...
        onClick={() => clearDeviceId(agent.id)}
      >
